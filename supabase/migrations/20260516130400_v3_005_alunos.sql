...
CREATE TABLE coaching.alunos (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  nome TEXT NOT NULL,
  email TEXT NOT NULL,
  avatar_url TEXT,
  concurso_id UUID REFERENCES coaching.concursos(id),
...
CREATE INDEX idx_alunos_concurso ON coaching.alunos(concurso_id) WHERE deletado_em IS NULL;
CREATE INDEX idx_alunos_role ON coaching.alunos(role);

CREATE TRIGGER trg_alunos_atualizado
  BEFORE UPDATE ON coaching.alunos
