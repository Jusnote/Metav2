-- UP: hotfix - remove completion_pct do INSERT em weekly_stats
-- weekly_stats.completion_pct é GENERATED ALWAYS AS (...) STORED no schema atual,
-- então tentar inserir valor literal falha com:
--   "cannot insert a non-DEFAULT value into column 'completion_pct'"
-- Re-define gerar_cronograma_v2 idêntico ao 20260515120600 exceto pela linha
-- ofensiva no INSERT INTO weekly_stats.
-- DOWN: rollback pra versão da migration 20260515120600.
...
BEGIN
  ...
  -- weekly_stats: SEM completion_pct (GENERATED ALWAYS)
