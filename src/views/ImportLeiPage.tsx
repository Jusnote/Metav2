...
import { parsePlainText, LEVEL_ORDER } from '@/lib/lei-parser';
import { convertToPlateJson, getExportStats } from '@/lib/lei-to-plate';
...
const STEPS: { key: ImportStep; label: string; icon: React.ReactNode }[] = [
  { key: 'paste', label: 'Colar Texto', icon: <FileText className="h-4 w-4" /> },
...
export default function ImportLeiPage() {
  ...
  const handleParse = useCallback(() => {
    ...
    setStep('hierarchy');
  }, [rawText, metadata.sigla, metadata.id]);

  // --- Step 2 → 3: Convert to Plate ---
  const handleConvert = useCallback(() => {
    if (!parseResult) return;
    const data = convertToPlateJson(parseResult, metadata);
    setExportData(data);
    setStep('review');
  ...
  const handleUpload = useCallback(async () => {
    ...
    // First click shows confirmation, second executes
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Cole o texto plano de uma lei (Ctrl+A, Ctrl+C do Planalto) e importe para o Supabase.
        </p>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-3">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">Texto da Lei</CardTitle>
            <CardDescription>
              Abra a lei no Planalto, pressione Ctrl+A depois Ctrl+C, e cole aqui.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
              ...
              placeholder="Cole o texto plano da lei aqui..."
      ...
      <div className="space-y-3">
        ...
        <Button onClick={onNext} disabled={!rawText.trim()} className="w-full">
          Analisar Texto
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Estrutura detectada automaticamente. Revise se está correta.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  if (isRoot && !hasChildren) {
    return (
      <p className="text-sm text-muted-foreground italic">
        Nenhuma estrutura hierárquica detectada. A lei pode não ter divisões (PARTE, TÍTULO, etc.).
      </p>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <StatCard label="Vigentes" value={stats.vigentes} color="text-green-600" />
        <StatCard label="Revogados" value={stats.revogados} color="text-red-600" />
        <StatCard label="Com Epígrafe" value={stats.withEpigrafe} />
        <StatCard label="Com Anotações" value={stats.withAnnotations} />
      </div>
            ...
            <CardContent className="p-0">
              <ScrollArea className="h-[500px]">
                <div className="divide-y">
                  {exportData.artigos.map((art) => (
                    <ArticlePreview
                      key={art.id}
                      article={art}
                      expanded={expandedArticle === art.id}
                      onToggle={() =>
                        setExpandedArticle(expandedArticle === art.id ? null : art.id)
                      }
                    />
                  ))}
                </div>
            ...
            <CardContent className="p-0">
              <ScrollArea className="h-[500px]">
                <pre className="p-4 text-xs font-mono whitespace-pre-wrap">
                  {JSON.stringify(exportData.artigos.slice(0, 5), null, 2)}
                  {exportData.artigos.length > 5 && (
                    `\n\n... e mais ${exportData.artigos.length - 5} artigos`
                  )}
                </pre>
              </ScrollArea>
            </CardContent>
