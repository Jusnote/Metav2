...
export function LeiSecaPageLegacy() {
  ...
  const { data: bookmarksSet } = useDispositivoBookmarks(currentLeiId);
  ...
  const handleToggleStatus = useCallback(
    (dispositivoId: string, status: DispositivoStatus | null) => {
      if (!leiIdRef.current) return;
      toggleStatusRef.current.mutate({
        dispositivoId,
        leiId: leiIdRef.current,
        status,
      });
    },
    []
  ...
  const handleToggleBookmark = useCallback((dispositivoId: string) => {
    if (!leiIdRef.current) return;
    toggleBookmarkRef.current.mutate({
      dispositivoId,
      leiId: leiIdRef.current,
    });
  }, []);
