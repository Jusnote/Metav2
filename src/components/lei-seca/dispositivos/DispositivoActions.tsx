...
interface DispositivoActionsProps {
  ...
  hasNote?: boolean;
  children: (gutter: React.ReactNode, below: React.ReactNode) => React.ReactNode;
}
...
export function DispositivoActions({
  ...
  // Gutter — goes inside the flex row
  const gutter = (
    <div
  ...
  // Below — footer + report modal, goes outside the flex row
  const below = (
    <>
      {footerOpen && (
        <DispositivoFooter
