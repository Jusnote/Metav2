"""Qwen com reasoning DESLIGADO — confirma se mantém acurácia barateando. Português,
mesmo gold do Opus (reusa tax/portugues.place_eval.json). THROWAWAY."""
...
def run(reasoning_off):
    preds, tin, tout, t0 = {}, 0, 0, time.time()
    for b in batches:
        body = {"model": "qwen/qwen3.5-flash-02-23", "messages": [{"role": "user", "content": make_prompt(b)}],
    ...
    return ok, n, tin, tout, cost, time.time() - t0


for label, off in [("reasoning OFF", True), ("reasoning ON", False)]:
    ok, n, tin, tout, cost, dt = run(off)
    print(f"Qwen {label:14}: {ok}/{n} = {100*ok/max(1,n):.0f}% | out {tout:>6} tok | {dt:.0f}s | "
